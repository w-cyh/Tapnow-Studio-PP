...
自原作者 zhengxinlan1995-code 的 2.6.1 版本以来到 3.8.6 版本（截至 2026-02-09）共经历 至少 141 次 快速迭代与修复（含 85 个公开版本号及 56+ 次内部逻辑微测），包含 6 个 里程碑版本与 3 次 重构。目前已进化为功能较为完善的工作流设计生产平台。
...
### 🚀 最新稳定版 (Latest Sync)
*   **版本号**: **v3.8.6** (2026-02-09)
*   **主要更新**: 
    *    **Antigravity Edit 路由**：模型库新增 `自动/仅Text-to-Image/仅Edit`，支持“接图走 edit、不接图走文生图”。
    *    **多图并发聚合**：同一提示词支持 `1/2/4/9` 张并发并汇聚到同一预览窗口，支持可配置下发间隔缓解 429。
    *    **智能分镜升级**：支持 `LLM拆脚本/LLM拆小说/LLM自定义`、卡片/表格双视图、表格导入与提示词汇总回填。
    *    **模型库默认参数增强**：图片/视频默认参数、参数禁用位置统一、默认值调用链路完善。
    *    **保存与预览稳定性**：降级保存手势错误修复、Lightbox 导航与加载占位收敛。
...
4.  用 `http://127.0.0.1:9527/ping` 验证本地接收器健康。
