...
### V3.7.31 (Batch Queue & Navigation Fix) - 2026-01-10
**构建文件**: `Tapnow Studio-V3.7.31.html`
- **[P0 修复] 锁定镜头仍触发任务**: 首次批量生成时也跳过已锁定镜头（`outputEnabled=true`）。
- **[P1 修复] 键盘导航首次无效**: 切换历史组时始终重置 `selectedMjImageIndex` 为 0。

### V3.7.30 (Preview Backfill Fix) - 2026-01-10
**构建文件**: `Tapnow Studio-V3.7.30.html`
- **[P0 修复] 预览不回填**: 移除 `setTimeout(0)` 包装，直接调用 `setNodes` 确保状态更新不丢失。
- **[P0 修复] 转圈不停止**: 同上修复。
- **[调试] 详细日志**: 添加 `[V3.7.30 Debug]` 标识的调试日志。

### V3.7.29 (Batch Generation Stability) - 2026-01-09
**构建文件**: `Tapnow Studio-V3.7.29.html`
- **[P0 修复] 状态竞态覆盖**: 超时检测增加 `onlyIfStatus: 'generating'` 条件更新。
- **[P0 修复] 强制重试遗漏**: 强制重试时立即将状态重置为 `pending` 确保加入队列。
- **[修复] 模型联动假象**: 移除 localStorage fallback，只显示 shot.model。
- **[优化] Toast 位置**: top-4 改为 top-20，向下移动。
- **[功能] 分镜卡片右键预览**: 2x2格子/1+3布局/单图都支持右键菜单。
- **[功能] 右键下载按钮**: previewContextMenu 增加"下载"选项。
- **[功能] 卡住镜头三选项**: prompt 支持 终止/跳过/放弃 三种操作。
- **[修复] 异步回填完整**: 传递 output_images 全部4张图而非单张。
- **[修复] 状态更新稳定**: setTimeout(0) 代替 queueMicrotask。
...
| 使用场景 | 推荐版本 | 原因 |
|---------|---------|------|
| **日常使用** | V3.7.23 | 最新功能，错误处理最完善 |
| **大量 API Key** | V3.7.23 | 熔断保护，智能轮换 |
| **性能敏感** | V3.5.20 | 专项性能优化 |
